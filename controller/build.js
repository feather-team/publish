...
Build.fetchRepo = function(req, res) {
    var branch = req.query.branch_name,
        exec = child_process.exec,
        workpath = path.join(REPO_PATH, 'feather');

    var repos = [];

    var walk = function(dir, done) {
        var results = [];
        if(!branch) throw '分支名不能为空';
        fs.readdir(dir, function(err, list) {
            if (err) return done(err);
            var pending = list.length;

            if (!pending) return done(null, err);
            list.forEach(function(file) {
                var filepath = path.resolve(dir, file);
                fs.stat(filepath, function(err, stat) {
                    if (stat && stat.isDirectory()) {
                        exec(CMD.GREP_BRANCH + branch, { 
                            cwd: filepath 
                        }, function(err, output, stderr) {
                            if (err !== null) {
                                throw('查找分支错误 : ' + err);
                            }
                            if (output) results.push(file);
                            pending--;
    ...
    try {
        walk(workpath, function(pending, results) {
            if (!pending && results) {
