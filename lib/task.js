var spawn = require('child_process').spawn, exec = require('child_process').exec, path = require('path');
...
var Task = module.exports = function(info, quiet, out){
    ...
    var tid = setTimeout(function(){
        ...
        if(info.args[0] == 'release.sh'){
            exec("ps xua|grep 'sh release'|awk '{print $2}'|xargs -n1 kill");
        }
...
Task.gc = function(){
    ...
    return stores = stores.filter(function(task){
        return !task.closeTime || task.closeTime && (now - task.closeTime < Task.GC_TIME);
    });
};

'sh git'.split(' ').forEach(function(method){
    Task[method] = function(options, quiet, timeout){
        options.cmd = method;
        ...
        return Task(options, quiet, timeout);
    }
});